# VN AI Agent Office — A 3D Workspace for AI Agents
...
**VN AI Agent Office** is a _3D virtual office for AI agents_ you run on your own infrastructure.
...
> **Unofficial project.** VN AI Agent Office is an independent, community-driven project and is not affiliated with, endorsed by, or maintained by the OpenClaw team. OpenClaw is a separate project, and this repository is not the official OpenClaw repository.

## What you can do with VN AI Agent Office
...
## What VN AI Agent Office Is

VN AI Agent Office is the visualization and interaction layer.
...
This repository does not build the upstream runtimes themselves. It is the frontend, Studio, and adapter/proxy layer that connects to a runtime speaking the OpenClaw gateway protocol.

## Why It Exists

AI systems are becoming more capable, but their work is still usually hidden behind logs, terminal output, and dashboards.

VN AI Agent Office exists to make agent systems visible:
...
The current app already includes a substantial VN AI Agent Office surface:
...
- VN AI Agent Office does not install or build OpenClaw or Hermes for you.
- Before starting VN AI Agent Office against a real backend, make sure your chosen runtime is already running and that you know the gateway URL and token Studio should use.
- For a no-framework local office demo, run the bundled demo gateway instead.
- If you need a full cross-machine setup guide (OpenClaw + Tailscale + VN AI Agent Office), follow [`TUTORIAL.md`](TUTORIAL.md).
...
Studio now also persists the selected backend mode (`OpenClaw`, `Hermes`, `Demo`, `Local`, `VN Office`, or `Custom`) and
...
HTTP runtime seam, start your runtime first, then start VN AI Agent Office:

```bash
npm run dev
```

Then open `http://localhost:3000`, choose `Local runtime`, `VN Office runtime`,
...
The browser does not call that runtime directly. VN AI Agent Office proxies the
...
VN AI Agent Office uses two separate network hops:
...
3. Open VN AI Agent Office from the LAN/Tailscale address instead of `localhost`.
...
- [`TUTORIAL.md`](TUTORIAL.md): detailed step-by-step setup for OpenClaw + Tailscale + VN AI Agent Office.
...
- `INVALID_REQUEST` errors mentioning `minProtocol` or `maxProtocol` usually mean the gateway is too old for VN AI Agent Office protocol v3. Upgrade OpenClaw, use the Hermes adapter, or run `npm run demo-gateway`.
...
1. Keep VN AI Agent Office running locally on `http://localhost:3000`.
...
- The main VN AI Agent Office app stays on `localhost`, so your normal local office state and agent state remain intact.
- Spotify redirects to the `ngrok` callback URL.
- The callback page passes the auth code back to the open local VN AI Agent Office window.
...
For first-time SSH connections, VN AI Agent Office uses `StrictHostKeyChecking=accept-new` by default so a new host key can be trusted automatically. If you need stricter behavior, set `OPENCLAW_GATEWAY_SSH_STRICT_HOST_KEY_CHECKING=yes`, or set it to `no` only if you explicitly want to skip host key checks.
