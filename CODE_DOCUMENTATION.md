...
This file is the practical code map for VN AI Agent Office contributors.
...
VN AI Agent Office is the UI and local Studio/proxy layer around an existing OpenClaw Gateway.

- OpenClaw owns agent execution, sessions, tools, config, and runtime events.
- VN AI Agent Office owns visualization, local Studio settings, UI workflows, office rendering, and the same-origin WebSocket/API bridge.
...
Sometimes VN AI Agent Office behavior depends on the upstream event contract or session behavior. In those cases:
...
3. Apply changes in VN AI Agent Office unless the user explicitly asked for upstream OpenClaw work.
