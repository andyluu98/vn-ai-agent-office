  ...
  return (
    <div className="ui-topbar relative z-[180]">
      <div className="grid h-10 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center px-3 sm:px-4 md:px-5">
        ...
        <p className="truncate text-sm font-semibold tracking-[0.01em] text-foreground">VN AI Agent Office</p>
        <div className="flex items-center justify-end gap-1">
          {status !== "disconnected" ? (
            <span
              ...
              data-status={status}
              aria-label={`Cổng kết nối ${status}`}
            >
              {status === "connecting" ? "Đang kết nối" : "Đã kết nối"}
            </span>
          ) : null}
          <ThemeToggle />
          {showConnectionSettings ? (
            <div className="relative z-[210]" ref={menuRef}>
              ...
              >
                ...
                <span className="sr-only">Mở menu studio</span>
              </button>
              {menuOpen ? (
