...
> Backend-neutral VN AI Agent Office integration plan for OpenClaw, Hermes, Vera, and other runtimes.

## Recommendation

Do not treat PR #70 as the long-term integration architecture.

It is useful as a short-term compatibility shim and a source of a few good UX changes, but it does not make VN AI Agent Office backend-neutral. It keeps VN AI Agent Office OpenClaw-shaped and makes Hermes imitate OpenClaw.
...
2. Extract a backend-neutral runtime adapter inside VN AI Agent Office.
...
VN AI Agent Office should stop treating the browser gateway client as the backend abstraction.
...
The browser can still use WebSocket streaming from Studio, but the messages should be VN AI Agent Office-native runtime events rather than implicitly OpenClaw events.
...
If a provider does not support a surface, VN AI Agent Office should disable or hide the UI for it. It should not fake a successful write.
...
The Vera provider should map VN AI Agent Office agent identities to routed roles or lanes rather than pretending Vera is an OpenClaw gateway.

## Event Model

Current VN AI Agent Office expects OpenClaw-flavored `chat`, `agent`, and `presence` events.

That is too narrow for universal providers. Studio should normalize provider-native updates into a VN AI Agent Office event model with explicit semantics:
...
- Map VN AI Agent Office agents to Vera roles or lanes.
...
If Luke wants "VN AI Agent Office should support any backend cleanly", PR #70 should not be the mainline architecture.
...
- with VN AI Agent Office visualizing agents, runs, status, and streamed text,
- while preserving Vera-specific routing, lane, and model identity.

That gives VN AI Agent Office a broader identity:
