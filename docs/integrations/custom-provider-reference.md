...
> Reference implementation guide for plugging a non-OpenClaw, non-Hermes runtime into VN AI Agent Office through the upstream-safe `custom` provider seam.

## Goal

Show how a custom orchestration stack should plug into VN AI Agent Office without requiring:
...
- VN AI Agent Office sees `custom`
- the external runtime stays implementation-specific
- VN AI Agent Office core remains generic
...
A custom runtime should sit at the VN AI Agent Office boundary as an orchestrator-backed service.

That means:

- VN AI Agent Office should talk to one stable runtime boundary
...
The `custom` provider exists so those stacks can integrate cleanly without forcing VN AI Agent Office core to absorb stack-specific assumptions.

## Recommended Boundary

VN AI Agent Office should integrate with the custom runtime's orchestrator or gateway layer, not with its individual workers.
...
This gives the UI enough identity without forcing upstream VN AI Agent Office to branch on every runtime brand.
...
Do not claim support just because the backend can theoretically do something. Claim support only where the provider exposes a stable VN AI Agent Office-facing behavior.

## Agent Mapping

VN AI Agent Office agents should not be modeled as one fixed backend process each unless the runtime truly behaves that way.
...
VN AI Agent Office sessions should map to runtime conversations or execution threads, not to whichever backend storage structure happens to exist internally.
...
The provider should translate runtime activity into the same normalized VN AI Agent Office runtime events used elsewhere.
...
The custom provider is the right place to map richer internal runtime signals into VN AI Agent Office's public office state model.
...
VN AI Agent Office only needs the resulting office-safe state and maybe a public reason label.

## Relationship To Office Systems

The custom provider should support office systems without VN AI Agent Office needing to know the backend's internals.
...
3. Map runtime roles or routes into VN AI Agent Office agent identities.
...
This is enough to make a custom runtime useful in VN AI Agent Office without overexposing internals.
...
- runtime-specific branches throughout VN AI Agent Office core
