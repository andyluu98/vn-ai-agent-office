...
Define a clean `custom` runtime provider class for VN AI Agent Office.

This provider should let external orchestration stacks integrate with VN AI Agent Office through a stable seam without requiring:
...
- VN AI Agent Office probes `GET /health`, `GET /state`, and `GET /registry`
- chat uses a direct HTTP path to `POST /v1/chat/completions`
- browser traffic is proxied through VN AI Agent Office's same-origin
...
Not every useful runtime should have to become a named built-in provider in upstream VN AI Agent Office.
...
VN AI Agent Office’s runtime abstraction already points toward multiple providers:
...
It should not require upstream VN AI Agent Office to know:
...
This gives VN AI Agent Office enough identity for UI without hardcoding a brand into the provider class.
...
- upstream VN AI Agent Office gets `custom`
...
This keeps VN AI Agent Office stable even if the custom stack has richer private event semantics internally.
...
The custom provider is the right place for richer internal state to enter VN AI Agent Office.
...
The custom provider should be able to support office systems without VN AI Agent Office needing to know the backend’s internals.
...
- make no runtime-specific assumptions in core VN AI Agent Office

Actual runtime adapters can be implemented separately against that seam.

## Out of Scope For V1

- embedding proprietary internal orchestration logic in VN AI Agent Office core
...
- upstream VN AI Agent Office gains a clean extension seam
...
It gives VN AI Agent Office extensibility, gives Vera a clean path in, and avoids hardwiring a personal stack directly into the core product model.
